...
public class StructuralTest {
	...
	public void testHierarchy() throws ClassNotFoundException {
		Class<?> clazz = Class.forName(StructuralTest.class.getPackage().getName() + "." + clazzName);
		try {
			String superClazz = expectedClazz.getString("superclass");
			assertEquals("Does not extend class " + superClazz + ".", superClazz,
	...
	public void testConstructors() throws ClassNotFoundException {
		Class<?> clazz = Class.forName(StructuralTest.class.getPackage().getName() + "." + clazzName);
		try {
			JSONArray expectedConstructors = expectedClazz.getJSONArray("constructors");
	...
	private void checkConstructors(Class<?> clazz, JSONArray expectedConstructors) {
		...
		for (int j = 0; j < expectedConstructors.length(); j++) {
			...
			for (Constructor<?> constructor : constructors) {
				...
				for (int k = 0; k < expectedParameters.length(); k++) {
					if (!expectedParameters.get(k).equals(parameterTypes[k].getSimpleName())) {
			...
			if (!constructorExists) {
				assertEquals("Expected a constructor with the parameters " + expectedParameters + ".", true,
						constructorExists);
			}
		}
	}
	...
	private void checkFields(Class<?> clazz, JSONObject expectedFields) throws NoSuchFieldException {
	...
	private void checkType(String expectedType, Field actualField) {
		if (expectedType.contains("<")) {
			...
			assertEquals(expectedMainType, actualField.getType().getSimpleName());
	...
	private void checkMethods(Class<?> clazz, JSONObject jsonMethods) throws NoSuchMethodException {
		...
		while (methodsIterator.hasNext()) {
			...
			assertEquals("Wrong return type in method " + methodName + ".", returnType,
					actualMethod.getReturnType().getSimpleName());
		}
	}
	...
	private void checkModifiers(int actualModifiers, JSONArray expectedModifiers) {
		...
		for (int i = 0; i < expectedModifiers.length(); i++) {
			...
			assertEquals("Expected modifiers " + expectedModifiers + " but were " + actualModifiersString + ".", true,
					modifierIsSet);
		}
	}
